...
func CreateAddressGroupCmd() *cobra.Command {
	...
	addressGroupCmd.PersistentFlags().StringVarP(&addressGroupFlags.addrGroupName, "ag-name", "n", "测试", "地址组名称")
	// 功能说明：是否只预检此次请求
	// 取值范围：
...
// 通过地址组名称找到地址组的 ID，以便在更新、创建、列出规则等逻辑中使用。
func findAddressGroupID(sgName string) (string, error) {
